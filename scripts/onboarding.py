...
def generate_config(
    ...
    use_copilot = api_keys.get("USE_COPILOT") == "true"
...
llm:
  provider: {"copilot" if use_copilot else "openai"}  # copilot, openai, or anthropic
  use_copilot: {str(use_copilot).lower()}
...
copilot:
  ...
  sync_context: true
  use_as_llm: {str(use_copilot).lower()}
